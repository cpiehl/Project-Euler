...
procedure Euler11 is
	max : integer := 0;
	prod : integer := 1;
	type Vector is array (1 .. 20, 1 .. 20) of integer;
	matrix : Vector :=
		(
		 1 => ( 0, 2,22,97,38,15, 0,40, 0,75, 4, 5, 7,78,52,12,50,77,91, 8),
...
begin
	...
	for i in integer range 1..20 loop
		for j in integer range 1..16 loop
			prod := 1;
	...
	for i in integer range 1..16 loop
		for j in integer range 4..20 loop
			prod := 1;
			for k in integer range 0..3 loop
				prod := prod * matrix(i+k, j-k);
			end loop;
